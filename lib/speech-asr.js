...
export async function transcribeRabiSpeech(config, input, signal, dependencies = {}) {
  ...
  try {
    ...
    const policy = input.automatic === true ? await readRabiVoicePolicy(config, operationSignal, dependencies) : null
    if (policy && !policy.ok) return policy
    const model = policy ? policy.config?.asrModel : asrModelFromStatus(status)
    ...
    if (!policy || policy.config.language) form.append('language', policy ? policy.config.language : 'zh')
    if (policy?.config.prompt) form.append('prompt', policy.config.prompt)
...
/** Read the owning Rabi microphone policy without changing its capture state. */
export async function readRabiVoicePolicy(config, signal, dependencies = {}) {
  try {
    const operationSignal = AbortSignal.any([signal, AbortSignal.timeout(10000)])
    const base = await (dependencies.resolveManagerBase || resolveManagerBase)(config, operationSignal, dependencies, { allowDegraded: true })
    const fetcher = dependencies.fetch || globalThis.fetch
    const values = await Promise.all(['/api/speech/microphone/status', '/api/speech/playback/status'].map(async path => {
      const response = await fetcher(base + path, { signal: operationSignal, redirect: 'error' })
      const payload = await response.json()
      if (!response.ok || payload.code !== 0) throw new Error(payload.message || `HTTP ${response.status}`)
      return payload.data
    }))
    return { ok: true, config: values[0].config, playbackActive: Boolean(values[1].current) }
  } catch (error) {
...
export function createAsrHandler(config, dependencies = {}) {
  return async (request, response) => {
    ...
    if (request.method === 'GET') return send(200, await readRabiVoicePolicy(config, new AbortController().signal, dependencies))
