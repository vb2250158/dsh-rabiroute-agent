...
export function createPlanResources(config, cache, attachments, dependencies = {}) {
  ...
  const selectionFile = path.join(directory, 'selections.json')
  ...
  let running = null
  let failure = ''
  const readSaved = async name => {
    try { return JSON.parse(await readFile(name, 'utf8')) }
    catch (error) { if (error.code === 'ENOENT') return null; throw error }
  }
  const ready = (async () => {
    const [pending, saved] = await Promise.all([readSaved(file), readSaved(selectionFile)])
    if (pending) {
      if (pending.version !== 1 || !Array.isArray(pending.items)) throw new Error('Invalid plan resource queue.')
    ...
    if (saved) {
      if (saved.version !== 1 || !Array.isArray(saved.selections)) throw new Error('Invalid plan resource selection archive.')
      for (const entry of saved.selections) {
        if (!entry || typeof entry.sessionId !== 'string' || !entry.sessionId || typeof entry.roleId !== 'string' || !entry.roleId || typeof entry.planId !== 'string' || !entry.planId || (entry.stepId !== undefined && typeof entry.stepId !== 'string')) throw new Error('Invalid archived plan resource selection.')
        selections.set(entry.sessionId, { roleId: entry.roleId, planId: entry.planId, stepId: entry.stepId })
      }
    }
  })()
  // A broken queue remains visible and is never overwritten with an empty one.
  ready.catch(error => { failure = error.message })
  const writeSaved = async (name, value) => {
  ...
  const save = () => {
    const result = persistence.then(() => writeSaved(file, { version: 1, items: [...items.values()] }))
    persistence = result.catch(error => { failure = error.message })
    return result
  }
  const saveSelections = () => {
    const saved = { version: 1, selections: [...selections].map(([sessionId, selection]) => ({ sessionId, ...selection })) }
    const result = selectionPersistence.then(() => writeSaved(selectionFile, saved))
    selectionPersistence = result.catch(error => { failure = error.message })
    return result
  }
  const lock = work => {
  ...
  async function execute(args, exec) {
    ...
    return lock(async () => {
      if (args.action === 'select') {
        ...
        const previous = selections.get(session.id)
        selections.set(session.id, { ...target, stepId: args.stepId })
        try { await saveSelections() }
        catch (error) {
          if (previous) selections.set(session.id, previous)
          else selections.delete(session.id)
          throw error
        }
        const prior = selected.map(item => ({ item, target: item.target, requiresChoice: item.requiresChoice }))
        for (const item of selected) { item.target = target; item.requiresChoice = false }
        if (selected.length) {
          try { await save() }
          catch (error) {
            for (const entry of prior) { entry.item.target = entry.target; entry.item.requiresChoice = entry.requiresChoice }
            throw error
          }
        }
  ...
  return { capture, captureTool, execute, status, flush, whenReady: () => ready,
    async checkpoint() {
      try { await ready; await serial; await persistence; await selectionPersistence }
      catch (error) { failure = error.message }
    },
    async dispose() { closed = true; await serial; await running; await persistence; await selectionPersistence },
  }
}
