 ...
 */
function RabiPlanLauncher({ sessionId, t, openRabiPlanTab, getRabiPanelView }) {
...
function createWorkspacePlanStore({ fetcher = (...args) => fetch(...args), Stream = globalThis.EventSource, debounceMs = 250 } = {}) {
...
function RabiAdvanceDialog({ cwd, roleIds, mode, t: parentT, onClose }) {
  ...
  return React.createElement(Modal, { open: true, title: t(mode), closeLabel: t('close'), onClose: () => { bulkController.current?.abort(); onClose() }, className: 'rabi-advance-dialog' },
    React.createElement('div', { style: { display: 'grid', gap: 10, minWidth: 0 }, 'data-rabi-advance': mode },
      ...
      !data ? error ? React.createElement(Button, { size: 'sm', onClick: () => setReload(value => value + 1) }, t('retry')) : React.createElement('div', { role: 'status' }, t('loading')) : mode === 'settings' ? React.createElement(React.Fragment, null,
        React.createElement('p', { style: { margin: 0, fontSize: 13, lineHeight: 1.5 } }, t('mechanism')),
        React.createElement('strong', null, t('globalDescription')),
        ...
        React.createElement('p', { style: { margin: 0, fontSize: 12, opacity: 0.8 } }, t('ruleDescription')),
        React.createElement('div', { style: { maxHeight: '52vh', overflow: 'auto', display: 'grid', gap: 8 } }, ...data.statuses.filter(status => status.state === 'enabled').map(status => {
          ...
          return React.createElement('section', { key: status.key, style: { border: '1px solid var(--dsw-alias-border-l4)', borderLeft: '3px solid ' + status.palette.accent, borderRadius: 6, padding: 8 } },
            React.createElement('div', { style: { display: 'flex', gap: 8, justifyContent: 'space-between' } },
              React.createElement(Button, { size: 'sm', variant: 'ghost', 'aria-expanded': openStatus === status.key, onClick: () => setExpanded(openStatus === status.key ? '' : status.key) }, status.label + ' · ' + t('editPrompt')),
              toggle(t('rule'), rule.enabled, enabled => update(status.key, { enabled }), inactive)),
            openStatus === status.key ? React.createElement('div', { style: { display: 'grid', gap: 8 } },
              React.createElement('div', { style: { fontSize: 12, opacity: 0.8 } }, t('statusDescription') + ' · ' + status.description),
              React.createElement('label', null, t('prompt'), React.createElement('textarea', { value: rule.prompt, disabled: busy || inactive, 'aria-label': status.label + ' ' + t('prompt'), placeholder: t('prompt'), onChange: event => update(status.key, { prompt: event.target.value }) })),
...
function RabiWorkspacePlanDialog({ cwd, t, store, openSession, onClose }) {
...
function RabiWorkspacePlanLauncher({ cwd, t, store, openSession }) {
