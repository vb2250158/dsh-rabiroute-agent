...
import { rabiPlanUrl } from './plan-panel.js'
...
export function createWorkspacePlans(config, listSessions, dependencies = {}) {
  ...
  const base = dependencies.base ?? (signal => resolveManagerBase(config, signal))
  ...
  return async function read(params, signal) {
    ...
    if (!roles.size) return { items: [], total: 0, nextCursor: '', facets: { statuses: [], tags: [] } }
    ...
    const pages = await Promise.all([...roles].map(async ([roleId, routeId]) => {
      const result = await json('/api/roles/' + encodeURIComponent(roleId) + '/plans/query', { method: 'POST', body: JSON.stringify({
        ...
        statuses: params.getAll('status'), tags: params.getAll('tag'),
    ...
    const managerBase = await base(signal)
    ...
    return { items: selected.map(plan => {
      ...
      return { id: plan.id, roleId: plan.roleId, title: plan.title, currentStep: plan.currentStep, presentation: plan.presentation,
        url: rabiPlanUrl(managerBase, plan.routeId, plan.id),
...
export function createWorkspacePlansHandler(read) {
  return async (request, response) => {
    ...
    try { send(200, { code: 0, data: await read(new URL(request.url, 'http://localhost').searchParams, controller.signal) }) }
