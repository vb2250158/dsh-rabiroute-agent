...
export async function findSessionPlans(fetcher, base, roleId, sessionId, signal) {
  ...
  for (let page = 0; page < MAX_PLAN_PAGES; page++) {
    ...
    if (!next || items.length === 0 || matched.length > 1) break
    cursor = next
  }
  return matched
}
...
export async function readRabiPlanPanel(config, sessionId, signal, dependencies = {}) {
  ...
  const roleId = String(binding?.data?.roleId || '').trim()
  if (!roleId) return unavailablePanel('unbound')
  const bound = await (dependencies.findSessionPlans || findSessionPlans)(fetcher, base, roleId, id, signal)
  if (bound.length === 0) return unavailablePanel('no-plan', roleId)
  if (bound.length > 1) {
  ...
  const gateways = await readJson(fetcher, base + GATEWAYS_PATH, signal)
