...
export function attachRabiQuestionExtras(root, options) {
  if (!root) return () => {}
  const cleanups = [...root.querySelectorAll('textarea')].map(field => enhanceCustomField(field, options))
...
export function registerRabiQuestionComposer(ctx) {
  ...
  ctx.slots.inject('conversation.composer', () => {
    const original = ctx.slots.entries('conversation.composer').find(entry => entry.options.locale === 'question')
    if (!original) throw new Error('rabiroute-agent requires the official question composer')
