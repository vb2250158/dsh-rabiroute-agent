...
export function installPlanResources(ctx, config, cache) {
  ...
  ctx.on('tools/pre-execute', (exec, next) => {
    ...
    const selected = service.status(sessionId).selected
    ...
    if ((plans.length > 1 && !selected) || (plans.length && mutation && !selected?.stepId)) return Promise.resolve({ kind: 'deny', reason: '先调用 rabiroute_plan_resources select 明确本轮 planId 与 stepId，再执行任务；用户附件用 itemIds 指定归属。' })
