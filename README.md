...
工作区 Rabi 计划弹窗提供“检查推进”和“推进设置”。规则按人格工作区保存，每种计划状态分别设置提示词、触发条件、动作、冷却时间和当前步骤次数上限。自动推进默认关闭；开启后监听计划及反馈变化、会话结束，并可选择启动检查和到期检查。检查页先展示可推进项和实际投递提示词，选中后只向原绑定 DSH 会话排队投递；“推进所有非运行中的会话”逐页检查当前人格工作区，每个符合规则的空闲原会话本轮最多投递一次，并显示处理进度。待审批、暂停、终态、绑定不匹配和运行中会话会跳过。Rabi 保存派发回执，避免同一计划变化重复投递；结果不明确时停止重试并等待核对。

推进设置页直接说明触发、检查与原会话排队投递的过程，并默认展开首个可配置状态，显示人格状态说明和该状态的可编辑推进提示词。保存走 Rabi 人格工作区规则接口；状态说明仍由人格工作流统一维护。实际提示词由 Rabi 合并状态说明、规则提示词、计划及步骤身份与授权约束，Agent 收到后重新读取最新计划。
...
数据只有一条路径。浏览器请求同源只读路由 `GET /rabiroute/plan-panel?sessionId=<DSH 会话 ID>`，Host 侧发现 Manager、核对 `/meta` 身份后依次读只读接口——`GET /api/codex-hook/sessions/<会话>` 取绑定角色；没有 Hook 人格记录时，从 `GET /api/gateways?summary=1` 取得去重后的角色，并行分页查询其当前计划。按完整会话 ID 匹配任务或秘书绑定，不创建人格绑定。存在 Hook 人格记录时，分页读取该角色的**当前计划摘要**并按 Rabi 的判据匹配绑定（`view=current`，每页 200 条摘要，最多 8 页），`GET /api/gateways?summary=1` 把角色映射到路由——再返回页面地址。不读计划正文，不读整份目录（那是按百条、每次上百 KB 的量级），不写入任何状态。浏览器在当前插件生命周期内按会话保留已确认的面板解析结果，不因切换多个会话逐出；浏览器重载时从会话存储还原计划身份和无凭据的地址信息，再在后台核对。相关计划事件只核对受影响的会话；失败时显示上次确认的结果与过期提示，手动重新加载会强制重读。带凭据的地址不写入浏览器存储。
