...
数据只有一条路径。浏览器请求同源只读路由 `GET /rabiroute/plan-panel?sessionId=<DSH 会话 ID>`，Host 侧发现 Manager、核对 `/meta` 身份后依次读只读接口——`GET /api/codex-hook/sessions/<会话>` 取绑定角色；没有 Hook 人格记录时，从 `GET /api/gateways?summary=1` 取得去重后的角色，并行分页查询其当前计划。按完整会话 ID 匹配任务或秘书绑定，不创建人格绑定。存在 Hook 人格记录时，分页读取该角色的**当前计划摘要**并按 Rabi 的判据匹配绑定（`view=current`，每页 200 条摘要，最多 8 页），`GET /api/gateways?summary=1` 把角色映射到路由——再返回页面地址。不读计划正文，不读整份目录（那是按百条、每次上百 KB 的量级），不写入任何状态，也不缓存绑定。
