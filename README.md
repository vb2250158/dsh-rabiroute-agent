...
DSH 通过公开 `conversation.chat.assistant-actions` 插槽呈现按钮，通过同源 `POST /rabiroute/speech` 适配 Manager。每次点击先发现并校验 Manager，再读取 `GET /api/speech/status`；仅在服务在线且存在 `defaults.tts` 时调用 `POST /api/speech/tts`，使用当前默认模型和 `play: true`。声线及其它未指定参数由 RabiSpeech 默认配置解析。语音在 Rabi 所在主机的共享队列播放，浏览器不另开播放器、不自动启动服务、不保存第二套 TTS 设置。
