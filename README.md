...
> 连接优化已随 **0.1.5** 发布，并通过官方入口的隔离安装及只读连接验证。**0.2.0** 增加右侧栏 Rabi 计划面板，已完成源码测试与真实 Manager 只读探测。源码测试、正式环境安装、运行中工具、Hook 和真实业务投递分别验收，不能用隔离验证代替正式环境部署。
...
会话在 Rabi 侧绑定了计划时，右侧栏出现「Rabi 计划」页：进入该会话会自动打开一次，之后由会话标题旁的「计划」按钮随时重开。面板渲染的不是重画的计划，而是 **Rabi 自己的单计划视图**（`#/routes/<路由>/plan/<计划 ID>`）；计划的步骤、反馈、审批与附件全部由 Rabi 呈现和驱动，DSH 不持有第二份。
