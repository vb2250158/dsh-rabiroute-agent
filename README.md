...
回复底部操作栏增加喇叭图标按钮。点击后只朗读该条已完成回复的正文文本，不含思考、工具调用或其他消息；正文为空时不显示按钮，超过 Rabi 的 10000 字符限制时明确拒绝，不截断。正文中的 Markdown 原文交给 TTS，不修改会话日志。

DSH 通过公开 `conversation.chat.assistant-actions` 插槽呈现按钮，通过同源 `POST /rabiroute/speech` 适配 Manager。每次点击先发现并校验 Manager，再读取 `GET /api/speech/status`；仅在服务在线且默认提供方已启用且声明了默认模型 时调用 `POST /api/speech/tts`，使用 `defaults.tts` 对应的 `providers.tts` 记录里的模型（`提供方/模型`）和 `play: true`。声线及其它未指定参数由 RabiSpeech 默认配置解析。语音在 Rabi 所在主机的共享队列播放，浏览器不另开播放器、不自动启动服务、不保存第二套 TTS 设置。

请求携带当前 DSH 会话 ID，供 Rabi 自己的语音记录关联；只转发正文、会话和 Rabi 返回的默认模型，不接受任意 API 地址或播放参数。提交期间按钮禁用；收到 `x-rabispeech-playback-job` 才显示已入队，这不等于已经播完。超时、断网或缺少回执时报告结果未确认，不自动重试。请求预算沿用 `requestTimeoutMs`；切换消息会取消等待，已提交的播放仍由 Rabi 持有。

Rabi 无法直接往 DSH 回复操作栏添加按钮，因此本插件仅承担点击入口、指定回复取文与同源通信。缺少插件不会改变 Rabi 公共 TTS 能力；服务关闭或 Manager 不可达时显示原因。源码行为测试与构建已覆盖本入口；正式安装、实际页面及出声需单独验收。
