...
Returning to a session immediately reuses its verified binding and page address; opening other sessions does not evict them. Plan events reconcile only affected sessions in the background. If reconciliation fails, the last verified page remains visible with a stale notice. Rabi Web still renders the panel: it immediately shows a browser-session snapshot, then conditionally updates it by view revision.
