...
import { playRabiSpeech, createSpeechHandler } from '../src/speech.js'
...
const play = dependencies => playRabiSpeech({}, input, new AbortController().signal, dependencies)
...
test('invalid text is rejected before discovery', async () => {
  for (const text of ['', ' ', 'x'.repeat(10001)]) {
    const deps = backend()
    assert.equal((await playRabiSpeech({}, { ...input, text }, new AbortController().signal, deps)).reason, 'bad-request')
    assert.equal(deps.calls.length, 0)
  }
})
test('lost receipt and failed writes stay uncertain without replay', async () => {
  for (const synth of [() => { throw new Error('network lost') }, () => new Response('audio'), () => Response.json({ message: 'failed' }, { status: 503 })]) {
    ...
    assert.equal((await play(deps)).reason, 'uncertain')
...
test('real HTTP adapter rejects malformed, oversized and cross-origin requests without playback', async t => {
  ...
  assert.equal((await response.json()).playbackJob, 'job-one')
})

test('speaker action reads the exact message text, excludes reasoning and prevents double clicks', async () => {
  const states = [], calls = []
  let cursor = 0, finish
  const context = { AbortController, Button: 'Button', fetch: (url, init) => { calls.push({ url, init }); return new Promise(resolve => { finish = resolve }) }, React: {
    createElement: (type, props, ...children) => ({ type, props, children }),
    useState: initial => { const i = cursor++; if (!(i in states)) states[i] = initial; return [states[i], value => { states[i] = value }] },
    useRef: initial => { const i = cursor++; return states[i] ??= { current: initial } }, useEffect: () => {},
  } }
  ...
  const snapshot = { eventNodes: [{ kind: 'assistant', messageId: 'other', blocks: [{ kind: 'text', text: 'Other reply' }] }, { kind: 'assistant', messageId: 'wanted', blocks: [{ kind: 'reasoning', text: 'Private reasoning' }, { kind: 'text', text: 'The reply' }, { kind: 'tool-call', name: 'test' }] }] }
  ...
  const playing = button.props.onClick()
  await button.props.onClick()
  assert.equal(calls.length, 1)
  assert.equal(JSON.parse(calls[0].init.body).text, 'The reply')
  assert.equal(render().children[0].props.disabled, true)
  finish(Response.json({ ok: true, playbackJob: 'job-one' }))
  await playing
  assert.equal(render().children[1].children[0], 'queued')
})
