...
test('registers all tools without contacting Host at registration', () => {
  const list = [], sections = []
  const ctx = { tools: { register: t => list.push(t) }, systemPrompt: { section: s => sections.push(s) }, inject: (_, fn) => fn(ctx), on() {} }
